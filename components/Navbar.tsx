...
export default function Navbar() {
  ...
  return (
    <nav
      className="fixed top-0 left-0 right-0 z-50 bg-amber-50/90 backdrop-blur-md"
            ...
            {loading ? (
              ...
              <div className="flex items-center gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-stone-600 text-sm">{user.username}</span>
                  {user.mbtiType && (
                    <span
                ...
                <button
                  onClick={logout}
                  className="text-stone-500 hover:text-stone-800 text-sm px-3 py-1.5 rounded-lg border border-stone-300 hover:border-stone-400 transition-all"
                >
                  ログアウト
                </button>
              </div>
              ...
              <>
                <Link
                  href="/login"
                  className="text-stone-600 hover:text-stone-800 text-sm px-4 py-2 rounded-lg border border-stone-300 hover:border-indigo-400 transition-all"
                >
                  ログイン
                </Link>
                <Link
              ...
              {user ? (
                <>
                  ...
                  <button
                    ...
                    className="text-left text-stone-500 hover:text-stone-800 text-sm px-2 py-1"
                  >
                    ログアウト
                  </button>
                </>
                ...
                <>
                  <Link href="/login" className="text-stone-600 text-sm px-2 py-1" onClick={() => setMenuOpen(false)}>
                    ログイン
                  </Link>
...
function NavLink({ href, children }: { href: string; children: React.ReactNode }) {
  return (
    <Link
      href={href}
      className="text-stone-600 hover:text-indigo-600 text-sm font-medium transition-colors"
    >
      {children}
    </Link>
  )
...
function MobileNavLink({ href, children, onClick }: { href: string; children: React.ReactNode; onClick: () => void }) {
  return (
    <Link
      href={href}
      className="block text-stone-600 hover:text-stone-800 text-sm font-medium px-2 py-1 transition-colors"
      onClick={onClick}
    >
      {children}
    </Link>
