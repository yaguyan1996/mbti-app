...
5ステップの分析フレームワークは使わず、まず感情に寄り添い「そうだったんですね」「それは辛かったですね」と共感を示してください。ユーザーの気持ちを言語化して反射し、アドバイスよりも「あなたの気持ちを理解している」ことを伝えることを最優先にしてください。最後に「もう少し聞かせてもらえますか？」と促す質問を1つだけ添えてください。`,
    ...
    console.error('Consultation error:', JSON.stringify(error, null, 2))
    const err = error as { status?: number; message?: string; error?: { type?: string } }
    const status = err.status ?? 500
    let message = err.message ?? String(error)
    if (status === 401) message = 'APIキーが無効です'
    else if (status === 402) message = 'APIクレジットが不足しています'
    else if (status === 429) message = 'APIのレート制限に達しました。しばらく待ってから試してください'
    return NextResponse.json({ error: message || '相談の処理に失敗しました', status }, { status: 500 })
  }
}
